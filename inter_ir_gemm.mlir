...
#pipeline_layout = #hal.pipeline.layout<bindings = [#hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer>]>
...
module @module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  util.func private @_test_gemm_a4w4(%arg0: tensor<16384x8192xui8>, %arg1: tensor<16384x8192xui8>) -> tensor<16384x16384xbf16> {
    ...
    %cst = arith.constant dense<0.000000e+00> : tensor<16384x16384xbf16>
    %cst_0 = arith.constant dense<1.000000e+00> : tensor<16384x512xf8E8M0FNU>
    %0 = util.optimization_barrier %cst_0 : tensor<16384x512xf8E8M0FNU>
    %1 = util.optimization_barrier %cst_0 : tensor<16384x512xf8E8M0FNU>
    %2 = util.optimization_barrier %cst : tensor<16384x16384xbf16>
    ...
    %cast_4 = tensor.cast %2 : tensor<16384x16384xbf16> to tensor<?x?xbf16>
    %3 = hal.dispatch.extern "_ZN5aiter42f4gemm_bf16_per1x32Fp4_BpreShuffle_256x256E"[%c16384, %c16384](%cast, %cast_1, %cast_2, %cast_3, %cast_4) : (tensor<?x?xui8>{%c16384, %c8192}, tensor<?x?xui8>{%c16384, %c8192}, tensor<?x?xf8E8M0FNU>{%c16384, %c512}, tensor<?x?xf8E8M0FNU>{%c16384, %c512}, tensor<?x?xbf16>{%c16384, %c16384}) -> tensor<?x?xbf16>{%c16384, %c16384} count(%arg2: !hal.device, %arg3: index, %arg4: index) -> (index, index, index) {
      ...
      #executable_target_rocm_hsaco_fb1 ordinal(0) = [#hal.executable.object<{path = "/home/jincheye/aiter/hsa/gfx950/f4gemm/f4gemm_bf16_per1x32Fp4_BpreShuffle_256x256.co"}>]
