...
void benchmark_module(size_t reductionSize) {
    ...
    std::vector<char> hsacoVec = readFileIntoVector("topk_ukernel.c.hsaco");
    if (hipModuleLoadDataEx(&module, hsacoVec.data(), 0, nullptr, nullptr) != hipSuccess) {
    ...
    *((uint32_t*)kernelParam[4]) = static_cast<uint32_t>(k);
    ...
    for (uint32_t i = 0; i < recordRuns; ++i) {
        assert(hipModuleLaunchKernel(kernel, gridX, gridY, 1,
    ...
    CHECK_HIP_ERROR(hipMemcpy(outputIndices.data(), d_outputIndices, bytesOutIdx, hipMemcpyDeviceToHost));
    ...
    std::cout << "Average time per run: " << (elapsedTimeMs / recordRuns) << " ms/iter" << std::endl;
